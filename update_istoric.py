...
# === Încărcare fișier (LOGICĂ MODIFICATĂ) ===
@st.cache_data(show_spinner="Încărc datele...")
def load_data():
    if not os.path.exists(DATA_PATH):
        st.warning("🔄 Fișierul istoric lipsește. Încerc să rulez actualizarea datelor...")
        
        # 🛑 RULEAZĂ ACTUALIZAREA
        update_istoric.main() 
        
        # Verifică dacă s-a creat fișierul
        if not os.path.exists(DATA_PATH):
            st.error("⚠️ Nu am reușit să creez fișierul istoric. Te rog să **verifici cheia API în Streamlit Secrets** și să te asiguri că ai acces la Football-Data.org.")
            return pd.DataFrame()
...
if pagina == "📊 Istoric echipă":
    ...
    for _, row in echipa_df.iterrows():
        try:
            # Asigurăm conversia sigură, ignorând datele lipsă
